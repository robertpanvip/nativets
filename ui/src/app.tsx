...
// ---------------------------------------------------------------------------
// 事件委托 demo：click 沿挂载树冒泡（DOM 语义）
// ---------------------------------------------------------------------------

const [delegLog, setDelegLog] = createSignal("");
...
function DelegationCard(_props: Props): El {
    ...
    return (
        <Card title="事件委托 · click 冒泡（DOM 语义）">
            <div style={{ flexDirection: "column", gap: 8 }}>
                {bubbleZone}
                {stopZone}
                {text(() => delegLog() === "" ? "（还没有点击）" : delegLog(), { fontSize: 11, color: C.textSecondary })}
            </div>
        </Card>
    );
}
